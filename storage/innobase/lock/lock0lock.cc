                                  ...
                                  trx_t *trx, bool holds_trx_mutex)
{
  ...
  lock_t *hash= static_cast<lock_t*>(c_lock->hash);
  lock_t *prev= nullptr;
  ...
  {
    prev= hash;
    hash= static_cast<lock_t*>(hash->hash);
  }
  lock->hash= hash;
  if (prev)
    prev->hash= lock;
	...
	que_thr_t*		thr)	/*!< in: query thread */
{
  ...
  IF_WSREP(LockGGuard g(lock_sys.rec_hash, id, trx->is_wsrep()),
  ...
  {
    {
      ...
      {
        trx->mutex_lock();
        /* FIXME: check_trx= */ lock_cancel_waiting_and_release(lock);
        trx->mutex_unlock();
      }
    }
    /* FIXME: check for new deadlocks in check_trx */
    mysql_mutex_unlock(&lock_sys.wait_mutex);
  }
...
FIXME: @return transactions to check for deadlocks */
static void lock_rec_dequeue_from_page(lock_t *in_lock, bool owns_wait_mutex)
{
	...
	for (lock_t* lock = lock_hash.get_first(page_id);
		...
		if (const lock_t* c = lock_rec_has_to_wait_in_queue(lock)) {
			IF_WSREP(wsrep_assert_no_bf_bf_wait(c, lock, c->trx),);
			lock->trx->lock.wait_trx = c->trx;
			/* FIXME: deferred deadlock check for c->trx
			unless in_lock->mode() == LOCK_X */
		} else {
			/* Grant the lock */
...
restart:
  ...
  for (lock_t *lock= UT_LIST_GET_LAST(trx->lock.trx_locks); lock; )
  {
    ...
    if (!lock->is_table())
    {
      ...
      {
        /* FIXME: check_trx= */lock_rec_dequeue_from_page(lock, false);
        latch->release();
      }
    }
    else
    {
      ...
      {
        /* FIXME: check_trx= */lock_table_dequeue(lock, false);
        table->lock_mutex_unlock();
      }
    }
...
void lock_cancel_waiting_and_release(lock_t *lock)
{
  ...
  if (!lock->is_table())
    /* FIXME: check_trx= */lock_rec_dequeue_from_page(lock, true);
  else
  {
	...
	trx_t*	trx)	/*!< in/out: trx lock state */
{
  ...
  /* FIXME: new deadlock check for check_trx */
  mysql_mutex_unlock(&lock_sys.wait_mutex);
  trx->mutex_unlock();
  return err;
}
